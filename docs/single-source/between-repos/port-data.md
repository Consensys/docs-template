...
This guide explains how to port content from upstream repositories (like MetaMask docs) to this documentation site using a hybrid approach that combines runtime plugins and CI-based sync PRs.

## Overview

The porting system uses a **hybrid approach**:
...
This provides both automatic updates (via CI) and runtime flexibility (via plugins), with explicit reviewable changes and the ability to revert even after merge.
